...
public class GenericTest extends BaseTest {
    ...
    public void all(final String expression, final F<Long> fi, final F<Double> fd, final F<BigInteger> fbi) {
        test(expression, "i", (x, y, z) -> (int) check(fi.apply(x, y, z)));
        test(expression, "d", fd);
        test(expression, "bi", fbi);
    }

    public void allConst(final String expression, final Integer v) {
